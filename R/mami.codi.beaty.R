...
compute_harmony_perception <- function(x) {
  ...
  x %>% dplyr::mutate(
    ...
    roughness         = log2(1+.data$space_roughness + .data$time_roughness),
    ...
    space_dissonance  = log2(1 + .data$space_roughness + .data$space_periodicity), # spatial  energy
    time_dissonance   = log2(1 + .data$time_roughness  + .data$time_periodicity),  # temporal energy
