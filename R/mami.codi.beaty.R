...
compute_harmony_perception <- function(x) {
  ...
  x %>% dplyr::mutate(
    ...
    space_dissonance  = log2(.data$space_periodicity + .data$space_roughness), # spatial  energy
    time_dissonance   = log2(.data$time_periodicity  + .data$time_roughness),  # temporal energy
