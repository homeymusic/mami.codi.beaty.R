...
inline double round_to_precision(double value, int precision = 15) {
 ...
 DataFrame rational_fractions(const NumericVector& x,
   ...
   for (int i = 0; i < n; ++i) {
     double ideal;
     if (i == 0) {
       // for the very first element, compare it to itself
       ideal = round_to_precision(x[i] / x[i]);
     } else {
       // otherwise compare to the previous tone
       ideal = round_to_precision(x[i] / x[i - 1]);
     }
