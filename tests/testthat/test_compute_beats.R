test_that('M7 beats make sense', {
  ...
  M7_l = SPEED_OF_SOUND / (M7 %>% hrep::freq())
...
test_that('beats make sense at octave P8 with 2 harmonics', {
  ...
  beats = compute_beats(wavelength = SPEED_OF_SOUND/spectrum$x, amplitude = spectrum$y)
...
test_that('calculated beat matches actual beat',{
  ...
  wavelengths = SPEED_OF_SOUND / freqs
  ...
  expect_equal(beats_wavelength_spectrum$amplitude,
               c(1.95),
               tolerance = 0.1)
  calculated_beat_freq = SPEED_OF_SOUND / beats_wavelength_spectrum$wavelength
...
test_that('only low beats are returned',{
  ...
  expect_equal(beats_wavelength_spectrum$amplitude,
               c(2,2,2),
               tolerance = 0.1)
  calculated_beat_freqs = SPEED_OF_SOUND / beats_wavelength_spectrum$wavelength
