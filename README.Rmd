...
p = params %>% purrr::map(\(p) {
  ...
  print(plot_semitone_codi(chords, 'Consonance',
                           goal=experiment,sigma=sigma,include_points=T,
                           black_vlines=black_vlines,gray_vlines=gray_vlines))
  cat("  \n")
  
  print(plot_semitone_periodicity_space_time(chords, 'Periodicity',
                                 goal=experiment,sigma=sigma,include_points=T,
                                 black_vlines=black_vlines,gray_vlines=gray_vlines))
  cat("  \n")
  
  print(plot_semitone_roughness_space_time(chords,title = "Roughness",
...
For these example frequencies: 
$f = 850.28, \quad f_{ref} = 260.0$

We traverse the Stern–Brocot tree six levels down along the path *RRRRLL* landing us on the fraction $\frac{10}{3}$.
...
```{r, fig.height=8, fig.width=12, echo=F, results='asis', message=F}
  ...
  print(plot_error_hist(time_error, bins=31, 'major', uncertainty(), 
                        "Rational Approximation Error for Frequencies (5 Harmonic Dyads): " * Delta~f == abs(f - tilde(f))))
  cat("  \n")
  ...
  print(plot_error_hist(space_error, bins=31, 'minor', uncertainty(), 
                        "Rational Approximation Error for Wavelengths (5 Harmonic Dyads): " * Delta~lambda == abs(lambda - tilde(lambda))))
  cat("  \n")
