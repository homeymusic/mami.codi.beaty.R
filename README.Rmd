...
params = list(
  ...
  list(h=4, o=2.0,t='Bonang'), # alignment with roughness and Harmonicity
  ...
  list(h=10,o=2.64,t='ExtraStretched'), # future predictions
  list(h=10,o=1.52,t='ExtraCompressed'), # future predictions
...
p = params %>% purrr::map(\(p) {
  ...
  cat("  \n", description, '\n')
    ...
    p$t = paste("Sethares (1993) ~ OAEs: YES ~ ", p$t)
  
  print(plot_semitone_codi(chords, paste0(p$t, ' (N=', N,' participants) : Consonance: -(Harmonicity + Roughness)'),
                           goal=experiment,sigma=sigma,
                           black_vlines=black_vlines,gray_vlines=gray_vlines))
  cat("  \n")
  
  print(plot_semitone_periodicity_roughness(chords, paste0(p$t, ' (N=', N,' participants) : Consonance: -Harmonicity & -Roughness'),
                                            goal=experiment,sigma=sigma,
                                            black_vlines=black_vlines,gray_vlines=gray_vlines))
  cat("  \n")
  
  print(plot_semitone_periodicity_space_time(chords, paste0(p$t, ' (N=', N,' participants) : -Harmonicity: Place & Time'),
                                             goal=experiment,sigma=sigma,
                                             black_vlines=black_vlines,gray_vlines=gray_vlines))
  cat("  \n")
  
  print(plot_semitone_roughness_space_time(chords,title = paste0(p$t, ' (N=', N,' participants) : -Roughness: Place & Time'),
                                          goal=experiment, sigma=sigma,
                                           black_vlines = black_vlines,gray_vlines  = gray_vlines)
  )
  cat("  \n")
  ...
  print(plot_roughness_periodicity(chords, 
                                   min_semitone=min_semitone,max_semitone=max_semitone,
                                   include_extreme_labels=include_extreme_labels,
                                   paste0(p$t, ': -Roughness versus -Harmonicity'), sigma=sigma))
  cat("  \n")
...
```{r, fig.height=8, fig.width=12, echo=F, results='asis', message=F}
  ...
  coprimer::plot_stern_brocot_tree(3, "RRL", dark_mode=T)
  cat("  \n")
  
  coprimer::plot_stern_brocot_tree(4, "RRLL", dark_mode=T)
  cat("  \n")
```
...
```{r, fig.height=8, fig.width=12, echo=F, results='asis', message=F}
  ...
  print(plot_semitone_periodicity_model_compare(chords, 'Bonang: MaMi.CoDi.Beaty Harmonicity with Harrison & Pearce 2018',
...
m2 = mami.codi.beaty(c(60,61), num_harmonics=1, verbose=T)

plot(m2$source_spectrum[[1]],
     xlim = c(240, 280))
title("Pure Tone Minor Second Stimulus Spectrum")
cat("  \n")

plot(m2$frequency_spectrum[[1]] %>% sparse_fr_spectrum_from_frequency_spectrum(),
     xlim = c(240, 280))
title("Pure Tone Minor Second Stimulus + OAE Spectrum")
cat("  \n")
